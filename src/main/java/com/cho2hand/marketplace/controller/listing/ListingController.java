...
public class ListingController {
    ...
    public PageResponse<ListingResponse> search(
        ...
        return PageResponse.from(service.search(new ListingSearchRequest(keyword, categoryId, conditionId, locationId, sellerUserId, minPrice, maxPrice,
    ...
    public PageResponse<ListingResponse> recommendations(
            @RequestParam(required = false) @Positive Long brandId,
            @RequestParam(required = false) @Positive Long modelId,
            @RequestParam(required = false) @DecimalMin("0") BigDecimal minPrice,
            @RequestParam(required = false) @DecimalMin("0") BigDecimal maxPrice,
        ...
        return PageResponse.from(service.recommendations(brandId, modelId, minPrice, maxPrice, locationId, excludeId, page, size));
    }

    @GetMapping("/mine")
    public PageResponse<ListingResponse> mine(@AuthenticationPrincipal Long userId,
            @RequestParam(defaultValue = "0") @PositiveOrZero int page,
            @RequestParam(defaultValue = "20") @Min(1) @Max(100) int size) {
        if (userId == null) throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Authentication required");
        return PageResponse.from(service.mine(userId, page, size));
    }
